...
// ─── In-memory store (ใช้ได้ dev — production ควรเปลี่ยนเป็น Redis/Firebase) ───
const trips = {};       // tripCode -> tripData
const sessions = {};    // userId -> { tripCode, carId }
...
async function handleEvent(event) {
  if (event.type === "join") {
    // Bot ถูก add เข้ากลุ่ม — เก็บ groupId
...
app.post("/api/location", async (req, res) => {
  ...
  try {
    ...
    // 3. เช็คว่ามีคันอื่นเพิ่งยิงอำเภอเดียวกันภายใน 3 นาทีไหม
    ...
    if (trip) {
      ...
      if (recentOther) {
        // คันแรกยิงไปแล้ว → ส่งแค่ pill สั้น
...
// ─── Helper: อากาศ (Open-Meteo ฟรี ไม่ต้อง key) ───────────────
...
// ─── Helper: หาชื่อคันรถจาก userId + tripCode ──────────────────
...
function buildFullFlexMessage(carName, district, weather, traffic) {
  const rainBadge = weather.rain < 20 ? { color: "#0F6E56", bg: "#E1F5EE", text: "ไม่มี" }
    : weather.rain < 50 ? { color: "#633806", bg: "#FAEEDA", text: "มีบ้าง" }
    : { color: "#791F1F", bg: "#FCEBEB", text: "ตกแน่" };

  const uvBadge = weather.uv <= 5 ? { color: "#0F6E56", bg: "#E1F5EE", text: "ปกติ" }
    : weather.uv <= 7 ? { color: "#633806", bg: "#FAEEDA", text: "สูง" }
  ...
  const trafficContents = traffic
    ? [
        { type: "separator", margin: "sm" },
        {
          ...
          contents: [
            {
              type: "text", text: "จราจรถึงปลายทาง",
